$(document).ready(function(){
    ...
    $("#btn-enviar-foto").on('click',function(event){
        ...
        $("#form-upload-foto").ajaxForm({
            url:'./paginas/contactos/upload/execute_upload.php',
            ...
            success:function(data){
                ...
                if(tipoMsg == "concluido"){
                    ...
                    $('#foto-contacto').attr("src",caminho_foto+"?timestamp="+ new Date().getTime());
                    mensagem.show().attr("class","mb-3 alert alert-success").html(msg);
                }else if(tipoMsg =="aviso"){
                    mensagem.show().attr("class","mb-3 alert alert-warnig").html(msg);
                    preloader.hide();
                }else{
